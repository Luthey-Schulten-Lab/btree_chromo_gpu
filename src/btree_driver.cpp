...
void btree_driver::update_replicate_modified_params(std::string &rep_mod, std::string &command, std::vector<std::string> &params)
{
  ...
  else if (command == "translocate")
    {
      insert_replicate_modifier(rep_mod,params[0]);
    }
  else if (command == "load_loops")
    {
      insert_replicate_modifier(rep_mod,params[0]);
    }
  else if (command == "write_loops")
    {
      insert_replicate_modifier(rep_mod,params[0]);
    }
...
void btree_driver::prepare_command_requirements()
{
  ...
  // loop_prng_seed
  // number of required parameters
  N_param_reqs["loop_prng_seed"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["loop_prng_seed"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["loop_prng_seed"] = t_ls;
  ...
  // lock tests
  t_ls.clear();
  lock_tests["replicator_set_time"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["replicator_set_time"] = t_ls;

  // replicator_set_volume
  // number of required parameters
  N_param_reqs["replicator_set_volume"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["replicator_set_volume"] = t_ls;
  // lock updates
  t_ls.clear();
  t_ls.push_back(new_lock("rep_volume_present",true));
  lock_updates["replicator_set_volume"] = t_ls;

  // replicator_load_volume_protocol
  // number of required parameters
  N_param_reqs["replicator_load_volume_protocol"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["replicator_load_volume_protocol"] = t_ls;
  // lock updates
  t_ls.clear();
  t_ls.push_back(new_lock("rep_volume_present",true));
  lock_updates["replicator_load_volume_protocol"] = t_ls;

  // replicator_set_DnaA
  // number of required parameters
  N_param_reqs["replicator_set_DnaA"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["replicator_set_DnaA"] = t_ls;
  // lock updates
  t_ls.clear();
  t_ls.push_back(new_lock("rep_DnaA_present",true));
  ...
  // replicator_load_model
  // number of required parameters
  N_param_reqs["replicator_load_model"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["replicator_load_model"] = t_ls;
  // lock updates
  t_ls.clear();
  t_ls.push_back(new_lock("rep_model_present",true));
  lock_updates["replicator_load_model"] = t_ls;

  // replicator_run
  // number of required parameters
  N_param_reqs["replicator_run"] = 1;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("btree_initialized",true));
  t_ls.push_back(new_lock("rep_model_present",true));
  ...
  /////////////////////////////////////
  // SMC looping command requirements //
  /////////////////////////////////////


  // translocate
  // number of required parameters
  N_param_reqs["translocate"] = 2;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("btree_initialized",true));
  lock_tests["translocate"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["translocate"] = t_ls;

  // load_loops
  // number of required parameters
  N_param_reqs["load_loops"] = 1;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("btree_initialized",true));
  lock_tests["load_loops"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["load_loops"] = t_ls;

  // write_loops
  // number of required parameters
  N_param_reqs["write_loops"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["write_loops"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["write_loops"] = t_ls;
  ...
  // cylindrical_bdry
  // number of required parameters
  N_param_reqs["cylindrical_bdry"] = 4;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("BD_lengths_present",true));
  lock_tests["cylindrical_bdry"] = t_ls;
  // lock updates
  t_ls.clear();
  ...
  // simulator_delete_loops
  // number of required parameters
  N_param_reqs["simulator_delete_loops"] = 1;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("simulator_prepared",true));
  t_ls.push_back(new_lock("lmp_data_present",true));
  lock_tests["simulator_delete_loops"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["simulator_delete_loops"] = t_ls;

  // simulator_form_loops
  // number of required parameters
  N_param_reqs["simulator_form_loops"] = 1;
  // lock tests
  t_ls.clear();
  t_ls.push_back(new_lock("simulator_prepared",true));
  t_ls.push_back(new_lock("lmp_data_present",true));
  lock_tests["simulator_form_loops"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["simulator_form_loops"] = t_ls;
  ...
  // switch_DNA_bdry_attraction
  // number of required parameters
  N_param_reqs["switch_DNA_bdry_attraction"] = 1;
  // lock tests
  t_ls.clear();
  lock_tests["switch_DNA_bdry_attraction"] = t_ls;
  // lock updates
  t_ls.clear();
  lock_updates["switch_DNA_bdry_attraction"] = t_ls;
					 ...
					 std::vector<std::string> &params)
{
  ...
  // set the internal time of the replicator
    ...
    {
      error_code = replicator_set_time(params);
    }
  

  // set the volume of the replicator
  else if (command == "replicator_set_volume")
    {
      error_code = replicator_set_volume(params);
    }


  // load a time-dependent protocol for the volume
  else if (command == "replicator_load_volume_protocol")
    {
      error_code = replicator_load_volume_protocol(params);
    }


  // set the number of free DnaA
  else if (command == "replicator_set_DnaA")
    {
      error_code = replicator_set_DnaA(params);
    }
  ...
  else if (command == "replicator_load_model")
    {
      error_code = replicator_load_model(params);
    }
  ...
  // write loops
    ...
    {
      error_code = write_loops(params);
    }


  // set loop simulator prng seed
  else if (command == "loop_prng_seed")
    {
      error_code = loop_prng_seed(params);
    } 
  ...
  // write file containing the spherical boundary coordinates
  else if (command == "spherical_bdry")
    {
      error_code = spherical_bdry(params);
    }

  // write file containing the cylindrical boundary coordinates
  ...
  else if (command == "simulator_delete_loops")
  {
    error_code = simulator_delete_loops(params);
  }


  // load file containing loop parameters
  else if (command == "simulator_form_loops")
  {
    error_code = simulator_form_loops(params);
  }
  ...
  // switch on the DNA bdry attraction
  else if (command == "switch_DNA_bdry_attraction")
  {
      error_code = switch_DNA_bdry_attraction(params);
  }
...
int btree_driver::replicator_set_time(std::vector<std::string> &params)
{
  ...
  driver_replicator.set_time(stod(params[0]));
  return 0;
}


int btree_driver::replicator_set_DnaA(std::vector<std::string> &params)
{
...
int btree_driver::replicator_set_volume(std::vector<std::string> &params)
{
  // set the replicator's volume
  driver_replicator.set_volume(stod(params[0]));
  return 0;
}


int btree_driver::replicator_load_volume_protocol(std::vector<std::string> &params)
{
  // load a time-dependent volume protocol for the replicator
  driver_replicator.load_volume_protocol(params[0]);
  return 0;
}


int btree_driver::replicator_load_model(std::vector<std::string> &params)
{
...
int btree_driver::translocate(std::vector<std::string> &params)
{
  // driver_ls.steps(stoi(params[0]));
  if (params[1] == "T")
  {
    // driver_lmp_simulator.update_loop_bonds(true, driver_ls);
...
int btree_driver::load_loops(std::vector<std::string> &params)
{
  int size = driver_lmp_sys.get_N_mono();
...
int btree_driver::write_loops(std::vector<std::string> &params)
{
  driver_ls.write_state(params[0]);
  return 0;
}
...
int btree_driver::spherical_bdry(std::vector<std::string> &params)
{
  ...
  // driver_lmp_sys.set_boundary_radius(stod(params[0]));
  // driver_lmp_simulator.set_boundary_radius(stod(params[0]));
  return 0;
}


int btree_driver::cylindrical_bdry(std::vector<std::string> &params)
{
...
int btree_driver::switch_DNA_bdry_attraction(std::vector<std::string> &params)
{
    int e = switch_extra_potential("DNA_bdry_attraction",params[0]);
    return e;
}
...
int btree_driver::map_replication()
{
  ...
  std::cout << "Applying mono mapping" << std::endl;
...
int btree_driver::simulator_delete_loops(std::vector<std::string> &params)
{
  if (params[0] == "T")
  {
    driver_lmp_simulator.delete_loops(true, driver_ls);
  }
  else if (params[0] == "F")
  {
    driver_lmp_simulator.delete_loops(false, driver_ls);
  }
  else
  {
    std::cout << "ERROR: invalid switch" << std::endl;
...
int btree_driver::simulator_form_loops(std::vector<std::string> &params)
{
  if (params[0] == "T")
  {
    driver_lmp_simulator.form_loops(true, driver_ls);
  }
  else if (params[0] == "F")
  {
    driver_lmp_simulator.form_loops(false, driver_ls);
  }
  else
  {
    std::cout << "ERROR: invalid switch" << std::endl;
...
{
  int e = driver_ls.read_loop_params(params[0]);
  return e;
}
...
int btree_driver::simulator_run_loops(std::vector<std::string> &params)
{
  ...
  if (skip_runs == true)
    {
      // still increment timestep for testing
      driver_lmp_simulator.increment_Nt(stoul(params[2]));
      return 0;
    }
...
int btree_driver::loop_prng_seed(std::vector<std::string> &params)
{
  // seed the PRNG
  driver_ls.prng_seed(stoi(params[0]));
  return 0;
}
