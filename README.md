...
2) **Calling LAMMPS_DNA_model_kk**: In order to perform minimizations and dynamics with Kokkos, one should call LAMMPS_DNA_model_kk instead of LAMMPS_DNA_model. In other words, one must do `simulator_set_DNA_model:/path/to/LAMMPS_DNA_model_kk`. Doing so ensures that one calls the kk versions of pair_style lj/cut, pair_style soft, bond_style fene, bond_style harmonic, and angle_style cosine. It also calls atom_style angle instead of atom_style hybrid angle ellipsoid, and replaces angle_style harmonic at replication forks with angle_style cosine. It also does not add angle_style polytorsion, both because there is no Kokkos version for that, and also because we have not figured out how to perform energy minimizations with the polytorsion term (LAMMPS minimze cg/kk does not vary the bead orientations).
3) **btree_chromo directives required to run with Kokkos**: Besides doing `simulator_set_DNA_model:/path/to/LAMMPS_DNA_model_kk` as indicated above, one must also be sure to use the command `switch_ellipsoids:F`, prior to doing `write_LAMMPS_data_file`. This  (As mentioned, we have removed atom_style ellipsoid; this was done because there is no Kokkos version of atom_style ellipsoid, and also we are not using the twisting or aligning energy terms, thus removing the need for a representation with orientations).
